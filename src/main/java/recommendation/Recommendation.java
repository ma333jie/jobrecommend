...
public class Recommendation {
	...
	public List<Item> recommendItems(String userId, double lat, double lon){
		...
		Set<String> visitedFavItemIds = new HashSet<>();
		...
		for (Entry<String, Integer> keyword : keywordList) {
			...
			for (Item item : items) {
				if (!favoritedItemIds.contains(item.getItemId()) && !visitedFavItemIds.contains(item.getItemId())) {
					recommendedItems.add(item);
					visitedFavItemIds.add(item.getItemId());
				}
			}
		}
